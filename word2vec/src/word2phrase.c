...
long long train_words = 0,c,cn,b,bi[100];
real threshold = 100;
    ...
    while(1) {
		...
		break;
	      ...
	      a++;
           ...
           cn++;
           for (a = 0; a < cn; a++) {
	    for (b = 0; b < vocab_size ; b++) if (!strcmp( vocab[b].word, st[a])) break;
            if (b == vocab_size) b = -1;
      	    bi[a] = b;
      	    printf("\n %lld Word: %s  Position in vocabulary: %lld Frequency:: %lld \n\n",a, st[a], bi[a], vocab[bi[a]].cn);
      	    if (b == -1) {
...
void TrainModel() {
  ...
  while (1) {
    ...
    // I edit this statement to preserve infrequency words
    if (score > threshold && pb > 10) {
      fprintf(fo, "_%s", word);
      pb = 0;
      if( !strcmp(word,"sennenhund")) printf("bigram %s\n",word);	
    } else {
      fprintf(fo, " %s", word);
      if( !strcmp(word,"sennenhund")) printf("unigram %s\n",word);	
    }
    pa = pb;
  }
  fclose(fo);
